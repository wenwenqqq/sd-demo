...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        model.train()
        # print_gpu_memory(rank, epoch=epoch, mode="训练前")
        ...
        for batch in train_dataloader:
            ...
            if rank == 0:
                # print_gpu_memory(rank, epoch=epoch, mode="batch之间")
                progress_bar.set_postfix({"loss": epoch_loss / num_batches})

        # print_gpu_memory(rank, epoch=epoch, mode="训练后")
        average_train_loss = epoch_loss / num_batches

        # ========== 验证 ==========
        model.eval()
        # print_gpu_memory(rank, epoch=epoch, mode="验证前")
        val_loss = 0.0
        val_batches = 0
        with torch.no_grad():
            for batch in val_dataloader:
                ...
                val_batches += 1
        # print_gpu_memory(rank, epoch=epoch, mode="验证后")
